...
export default function OldCheckoutPaymentPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (!orderId) throw new Error('訂單建立失敗，無法取得訂單 ID');
      dispatch(
        pushMessage({
          text: '訂單建立失敗，無法取得訂單 ID',
          status: 'failed',
        })
      );
      ...
      dispatch(
        pushMessage({ text: '發生錯誤，已送出訂單失敗', status: 'failed' })
      );
    } finally {
